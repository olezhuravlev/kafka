...
public class TransactionProcessorTest {
    ...
    public void setUp() throws IOException {
        ...
        properties.putAll(Map.of(SCHEMA_REGISTRY_URL_CONFIG, "mock://schema-registry.kafkainaction.org:8080",
        ...
        topology = new TransactionProcessor(transactionsInputTopicName, transactionSuccessTopicName, transactionFailedTopicName, fundsStoreName).topology(streamsBuilder, transactionSerde, transactionResultSerde, fundsSerde);
        
        deposit100 = new Transaction(UUID.randomUUID().toString(), "1", new BigDecimal(100), DEPOSIT, "USD", "USA");
        
        withdraw100 = new Transaction(UUID.randomUUID().toString(), "1", new BigDecimal(100), WITHDRAW, "USD", "USA");
        
        withdraw200 = new Transaction(UUID.randomUUID().toString(), "1", new BigDecimal(200), WITHDRAW, "USD", "USA");
    }
    ...
    public void shouldCreateSuccessfulTransaction() {
        ...
        try (TopologyTestDriver testDriver = new TopologyTestDriver(topology, properties)) {
            ...
            final TestInputTopic<String, Transaction> inputTopic = testDriver.createInputTopic(transactionsInputTopicName, stringSerde.serializer(), transactionSerde.serializer());
            
            inputTopic.pipeInput(deposit100.getAccount(), deposit100);
            inputTopic.pipeInput(withdraw100.getAccount(), withdraw100);
            
            final TestOutputTopic<String, TransactionResult> outputTopic = testDriver.createOutputTopic(transactionSuccessTopicName, stringSerde.deserializer(), transactionResultSerde.deserializer());
    ...
    public void shouldBeInsufficientFunds() {
        ...
        try (TopologyTestDriver testDriver = new TopologyTestDriver(topology, properties)) {
            ...
            final TestOutputTopic<String, TransactionResult> failedResultOutputTopic = testDriver.createOutputTopic(transactionFailedTopicName, stringSerde.deserializer(), transactionResultSerde.deserializer());
            
            final TestOutputTopic<String, TransactionResult> successResultOutputTopic = testDriver.createOutputTopic(transactionSuccessTopicName, stringSerde.deserializer(), transactionResultSerde.deserializer());
